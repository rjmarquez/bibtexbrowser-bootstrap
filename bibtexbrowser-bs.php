...
<!--this is version v20150422 -->
URL: http://www.monperrus.net/martin/bibtexbrowser/
Feedback & Bug Reports: martin.monperrus@gmail.com

(C) 2015 Richard Marquez - Added HTML5, Bootstrap and no iframes (iframes do not work very well with responsiveness).
...
define('BIBTEXBROWSER','v20150422');
...
@define('TYPES_SIZE',100); // number of entry types per table
@define('YEAR_SIZE',200); // number of years per table
@define('AUTHORS_SIZE',300); // number of authors per table
...
//     echo $this->getKey();
  ...
  */
  ...
  function getUrlLink($iconurl = NULL, $label = 'url') {
...
function DefaultBibliographyStyle(&$bibentry) {
  ...
  if ($bibentry->hasField('note')) {
      $result .=  "<br /> (".$bibentry->getField("note").")";
  }
...
$("form").submit(function( event ) {
  ...
  $( "#principal" ).load( "index.php?search=" + text + "&bib=<?php echo $_GET[Q_FILE]; ?>");
});
...
$('a.menuitem').each(function() { // for each element of menu
  var itemurl = $(this);
  if (itemurl.attr('bibtexbrowser') === undefined)
  {
  itemurl.click(function(ev) { // we change the click semantics
    ev.preventDefault(); // no open url
      var bibtexEntryUrl = $(this).attr('href');
      $( "#principal" ).load( bibtexEntryUrl );
  });
  itemurl.attr('bibtexbrowser','done');
  } // end if itemurl.bibtexbrowser;
});
...
class MenuManager {
  ...
  function searchView() {
    ?>
    <form id="enviarforma" action="?" method="get">
      <input type="text" id="<?php echo Q_SEARCH; ?>" name="<?php echo Q_SEARCH; ?>" class="input_box" size="18"/>
 ...
 echo '<a '. $href .' target="_blank" class="menuitem">'. $item ."</a>\n";
...
class BibEntryDisplay {
  ...
  function displayOnSteroids() {
      ...
      if ($this->bib->hasField('url')) {
        $download = '<div class="bibentry-document-link"><a href="'.$this->bib->getField('url').'">View</a></div>';
      }
...
.sheader {
  font-weight: bold;
  background-color: #2c5aa0;/* #003366; */
  color: #ffffff;
  padding: 2px;
  margin-bottom: 0px;
  margin-top: 7px;
...
.btb-nav-title {
  background-color: #2c5aa0; /* #995124; */
  color: #FFFFFF;
  padding: 1px 2px 1px 2px;
}
...
if (method_exists($content, 'getTitle')) {
  echo "<h1 class=\"page-header\">" . $content->getTitle() . "</h1>";
}
?>
<?php
  $content->display();
...
class Dispatcher {
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: 20150422\n";
...
<html lang="en">
  ...
  <body>
    ...
    <div id="content" class="container-fluid">
      <div class="row">
        <div id="menuprincipal" class="col-sm-3 col-md-3 col-lg-3 sidebar">
        </div>
        <div  id="principal" class="col-sm-9 col-sm-offset-3 col-md-9 col-md-offset-3 col-lg-9 col-lg-offset-3 main">
	</div>
      </div>
    </div>
    ...
    <script>
    $.get('<?php echo '?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&menu'; ?>', function(data) {
        $("#menuprincipal").append(data);
    });

    $.get('<?php echo 'index.php?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&'.BIBTEXBROWSER_DEFAULT_FRAME; ?>', function(data) {
        $("#principal").append(data);
    });



    // document.querySelector('#prueba').innerHTML = el.innerHTML;
